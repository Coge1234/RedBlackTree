 ...
 * 7、内部插入接口方法定义：insert(RBNote node)
 * 8、修正插入导致红黑树失衡的方法定义：insertFixUp(RBTree node)
 * 9、测试红黑树正确性
 */
public class RBTree <K extends Comparable<K>, V>{
     ...
     *                          将爸爸节点进行一次左旋，得到LL双红的情景（4.2.1），然后指定爸爸节点为当前节点进行下一轮处理
     ...
     *      / \          / \
     *    ly   ry      lx  ly
     * 1、将x的右节点指向y的左子节点(ly)，将y的左子节点的父节点更新为x
    ...
    private void leftRotate(RBNode x) {
        ...
        if (x.parent != null) {
            ...
            // 说明x就是根节点,需要更新y的父节点为x的父节点，并将x的父节点指定子树(当前x的子树位置) 指定位y
            this.root = y;
     ...
     * 2、当y的父节点(不为空时)，更新x的父节点为y的父节点，更新y的父节点指定子节点(当前y的子树位置)为x
     * 3、更新y的父节点更新为x，更新x的右子节点更新为y
     */
    private void rightRotate(RBNode y) {
        ...
        // 2、y的父节点(不为空时)，更新x的父节点为y的父节点，更新y的父节点指定子节点(当前y的子树位置)为x
        if (y.parent != null) {
            x.parent = y.parent;
            if (y == y.parent.left) {
                y.parent.left = x;
            ...
            // 说明y就是根节点,需要更新x的父节点为y的父节点，并将y的父节点指定子树(当前y的子树位置) 指定位x
            this.root = x;
            this.root.parent = null;
        }
        // 3、更新y的父节点更新为x，更新x的右子节点更新为y
